...
const navItems = [
  ...
  { to: '/saved', label: 'Saved', icon: Heart, requiresAuth: true },
]
    ...
    <>
        ...
        <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-6">
          ...
          <nav className="flex items-center gap-1">
            {navItems.map(({ to, label, icon: Icon, requiresAuth }) => {
              const href = requiresAuth && !user ? '/login' : to
              const linkState = requiresAuth && !user ? { from: to } : undefined
              return (
                <Link
                  key={to}
                  to={href}
          ...
          <div className="flex items-center gap-2">
            {user ? (
              ...
              <Button asChild size="sm">
                <Link to="/login" state={{ from: '/add-viewpoint' }}>
                  <Plus className="h-4 w-4" />
                  Add spot
                </Link>
              </Button>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-40 border-t border-border bg-white/95 backdrop-blur md:hidden">
        <div className="mx-auto flex max-w-lg items-center justify-around px-2 py-2">
          ...
          <Link
            to={user ? '/add-viewpoint' : '/login'}
            state={user ? undefined : { from: '/add-viewpoint' }}
